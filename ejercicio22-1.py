...
cola_personajesMCU.arribo(personajes_MCU)
personajes_MCU = PersonajesMCU('Carol Danvers','Capitana','F')
cola_personajesMCU.arribo(personajes_MCU)
...
while(cantidad_elementos < cola_personajesMCU.tamanio()):
    ...
    if (dato.nombre_superheroe == 'Scott Lang'):
        print('El nombre del superhéroe del personaje Scott Lang es ', dato.nombre_personaje) 
     
    if (dato.nombre_personaje[0] == 'S'):
        print('Datos de los personajes que comienzan con S: ', 'Nombre personaje: ',dato.nombre_personaje,'Nombre de superheroe: ',dato.nombre_superheroe,'Genero: ',dato.genero)
   
    if(dato.nombre_personaje == 'Carol Danvers'):
    	print('Carol Danvers se encuentra en la cola y y su nombre de super heroe es ', dato.nombre_superheroe)
